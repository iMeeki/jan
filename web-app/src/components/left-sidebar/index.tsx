      ...
      <Sidebar variant="floating" collapsible="offcanvas">
        <SidebarHeader className="flex px-1">
          <div className={cn("flex items-center w-full justify-between", reserveLeft && "justify-end")}>
            {!reserveLeft && <span className="ms-2 font-medium font-studio">MixGPT</span>}
            <div className="flex items-center">
              {controlsOnLeft && (
                <span className="me-2 font-medium font-studio">Jan</span>
              )}
              {isLeftPanelOpen && <DownloadManagement />}
              <SidebarTrigger className="text-muted-foreground rounded-full hover:bg-sidebar-foreground/8! -mt-0.5 relative z-50 ms-0.5" />
            </div>
          </div>
          <NavTabs />
