			...
			>
				...
				<div
					className={cn(
						...
						variant === "floating" || variant === "inset"
							? "p-2 pe-0 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4))+2px)]"
...
>(({ className, enableDrag = true, ...props }, ref) => {
	...
	return (
		<button
			...
			className={cn(
				"absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:start-1/2 after:w-0.5 hover:after:bg-sidebar group-data-[side=left]:-end-4 group-data-[side=right]:start-0 sm:flex",
				...
				"[[data-side=left][data-collapsible=offcanvas]_&]:-end-1",
				"[[data-side=right][data-collapsible=offcanvas]_&]:-start-1",
				// Adjust for floating variant - match the rounded corners and padding
				"group-data-[variant=floating]:inset-y-2 group-data-[side=left]:group-data-[variant=floating]:-end-4 group-data-[variant=floating]:after:rounded-lg group-data-[variant=floating]:mt-4 group-data-[variant=floating]:h-[calc(100%-4rem)]",
				className,
			)}
			{...props}
		/>
